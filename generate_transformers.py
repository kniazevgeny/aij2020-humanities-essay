...
def main():
    ...
    logger.info(args)
    generated_sequences = []
    tests = json.load(open(args.filename, encoding="utf-8"))
    prompt_text = ""
    output = []
    for test in tests:
        result = {
        ...
        if "russian" in args.filename: 
            prompt_text = prompt_text[prompt_text.find("(1)"):].replace('\n\n', ' ')
        else:
        ...
        for generated_sequence_idx, generated_sequence in enumerate(output_sequences):
            ...
            result["essay"] = text[len(tokenizer.decode(encoded_prompt[0], clean_up_tokenization_spaces=True)) + 12 :]
